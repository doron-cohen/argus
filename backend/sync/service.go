...
// updateStatusFromSyncResult updates the status for a source based on sync result
func (s *Service) updateStatusFromSyncResult(index int, status *SourceStatus) {
	now := time.Now()
	status.LastSync = &now
